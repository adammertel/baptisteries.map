...
const distance = (cs1, cs2) =>
  Math.sqrt(
    Math.pow(cs1[0] - cs2[0], 2) + Math.pow(cs1[1] - cs2[1], 2)
  )
...
const init = () => {
  ...
  bins.map(bin => {
    ...
    const shapeCounts = {}
    shapes.map(shape => {
      shapeCounts[shape] = shapeCounts[shape]
        ? shapeCounts[shape] + 1
        : 1
    })
    ...
    cPie.map(cP => {
      ...
      cPieG
        ...
        .attr(
          'fill',
          cP.data.label === 'true' ? ciboriumColor : 'black'
        )
    })
  ...
  const legendH =
    (allShapes.length / 2) * legendItemH +
    legendPadding * 2 +
    headingH +
    200
  ...
  const hexLegendDates = Array(5)
    ...
    .map(
      (_, i) =>
        30 + hexDepths[0] + (i * (hexDepths[1] - hexDepths[0])) / 5
    )
  ...
  hexLegendDates.map((depth, hi) => {
    ...
    const color =
      depth !== 'unknown' ? binColors(depth) : defaultBinColor
    legendG
      .append('path')
      .attr('d', d => {
  ...
  const legendSizes = Array(legendSizeSteps)
    ...
    .map((_, i) =>
      parseInt(
        (maxInside / (legendSizeSteps * legendSizeSteps)) *
          ((i + 1) * (i + 1)),
        10
      )
    )
